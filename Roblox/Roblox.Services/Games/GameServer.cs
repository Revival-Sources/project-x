...
public class GameServerService : ServiceBase
{
    ...
    public GameServerJwt DecodeTicket(string ticket, string? expectedIpAddress)
    {
        ...
        if (value.t != ClientJoinTicketType) throw new ArgumentException("Invalid ticket CLIENT JOIN");
        if (IsExpired(value.iat))
        {
            throw new ArgumentException("Invalid ticket EXPIRED");
        }
        ...
        if (expectedIpAddress != null)
        {
            ...
            if (!ipOk)
            {
                throw new ArgumentException("Invalid ticket IP NOT OK");
            }
        }
    ...
    public GameServerTicketJwt DecodeGameServerTicket(string ticket)
    {
        ...
        if (value.t != ServerJoinTicketType) throw new ArgumentException("Invalid ticket SERVER JOIN");
        if (IsExpired(value.iat))
        {
            throw new ArgumentException("Invalid ticket EXPIRED GAMESERVER");
        }
    ...
    public async Task<GameServerGetOrCreateResponse> GetServerForPlace(long placeId)
    {
        ...
        if (true)
        {
            var job = CreateGameServerTicket(placeId, "85.215.209.73:53640");
            return new()
            {
                status = JoinStatus.Joining,
                job = job,
