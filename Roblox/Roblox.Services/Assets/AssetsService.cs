...
WHERE asset_type = :asset_type AND asset.id < :id AND NOT asset.is_18_plus ORDER BY asset.id DESC LIMIT :limit",
    ...
    public async Task<IEnumerable<AdvertisementEntry>> GetAdvertisementsByGroup(long groupId)
    {
        ...
        string query = @"
            SELECT " + UserAdColumns + @"
            FROM asset_advertisement
            WHERE target_id IN (
                SELECT id FROM asset WHERE asset.creator_type = 2 AND asset.creator_id = @id
            ) AND target_type = @asset_type OR (asset_advertisement.target_id = @id AND asset_advertisement.target_type = @group_type)";
        ...
        return await db.QueryAsync<AdvertisementEntry>(query, parameters);
    }
