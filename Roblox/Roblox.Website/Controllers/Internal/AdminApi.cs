...
public class AdminApiController : ControllerBase
{
    ...
    {
        // give commission
        await services.economy.IncrementCurrency(userSession.userId, CurrencyType.Robux, 15);
...
public async Task<dynamic> RunLottery()
{
    ...
    if (allItems.Count == 0)
        throw new StaffException("There are no items available for the lottery");
    ...
    log.Info("Sent message to the user picked {0}", randomUser.userId);
    ...
    log.Info("Sent message to the old asset owner {0}", randomItem.userId);
