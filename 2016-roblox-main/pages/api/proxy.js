...
const UrlUtilities = (() => {
  const getDomainFromUrl = (url) => {
    ...
    if (baseDomainParsed.type === ParseResultType.Listed) {
      ...
      if (baseDomainParsed.hostname === 'projex.zip') {
        return 'projex.zip';
      }
      throw new Error('The only allowed reserved domain type is projex.zip, got ' + baseDomainParsed.hostname);
    } else {
    ...
    for (const item of Object.getOwnPropertyNames(result.headers)) {
      ...
      if (item === 'set-cookie') {
        ...
        if (typeof value === 'string') {
          value = value.replace(/roblox\.com/g, 'projex.zip');
        } else {
          value.forEach((v, i, arr) => {
            arr[i] = v.replace(/roblox\.com/g, 'projex.zip');
          });
        }
      }
