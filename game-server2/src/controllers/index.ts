...
export default class CommandHandler extends StdExceptions {
	...
	private async startRcc(game: IGameEntry): Promise<void> {
		...
		if (!dockerEnabled) {
			const rccPath = conf.rcc || path.join(__dirname, 'C:\\services\\RCCService\\');
			const rccExecutable = rccPath + (os.platform() === 'win32' ? 'RCCService.exe' : 'rcc');
			rcc = cp.spawn(rccExecutable, ['-Console', '-PlaceId:48', "-port 64989", '-Content', conf.content || path.join(rccPath, './content/')], {
				cwd: rccPath,
				stdio: 'pipe',
			});
		}else{
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
	...
	public async removeFromRunningJobs(jobId: string) {
		console.log('[info] killing off job started');
		let rcc = await this.GetRccForRender();
		console.log('[info] got rcc for render');
		if (!rcc || !rcc.rccReference) {
			// fatal error
			console.error('fatal','RCC reference is not available - it was probably closed', new Error().stack);
			process.exit(1);
		}
		console.log('[info] killing off job');
		await this.openJob(rcc, 10 * 1000, rcc.rccReference.port, this.createShutdownRequest(jobId));
